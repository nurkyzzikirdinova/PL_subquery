...
where doctor_id in (select d.id
                    from doctors d
                    where department_id in (select d.id from departments d where d.department_name = 'Surgical'));  --successfully
--2. Получить список всех отделений, где работает врач с именем 'Miller’
select *
from departments d
where id in (select department_id from doctors where first_name = 'Miller'); --successfully
-- 3.Найти всех пациентов, у которых врач имеет стаж более 10 лет
select *
from patients p
where p.doctor_id in (select id from doctors d where experience > 10);--successfully
...
select *
from patients p
where p.doctor_id in (select id from doctors where doctors.first_name = 'Conrado' and doctors.last_name = 'Placide'); --successfully
-- 9.Получить список всех врачей, работающих в отделении 'Intensive' и обслуживающих больше 3 пациентов
select *
from doctors doc
where doc.department_id in (select d.id from departments d where d.department_name like 'Intensive'); --successfully
--10. Получить список всех отделений, в которых пациенты лечатся у врачей с опытом менее 5 лет
select distinct department_name
from departments
         join patients p on departments.hospital_id = p.hospital_id
         join doctors d on departments.id = d.department_id
where experience > 5;--successfully
-- 11.Найти всех пациентов, у которых врач имеет специализацию 'DERMOTOLOGIST’
select *
from patients
where doctor_id in (select id from doctors where specialization = 'DERMOTOLOGIST'); --successfully
...
where doctor_id in (select id
                    ...
                    where specialization =
                          (select specialization from doctors group by specialization order by count(*) asc limit 1)); --successfully
